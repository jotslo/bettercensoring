...
async def run(message, guild_settings):
    ...
    arguments = await functions.get_arguments("manage-embeds", message)
    ...
    if arguments[0] in ["on", "off", "enable", "disable"]:
        ...
        functions.set_data(
            "UPDATE guild_settings SET filter_embeds = (?) WHERE guild_id = (?)",
            (is_enabled, guild_settings["guild_id"], )
        )
        await functions.send_embed(
            message.channel,
            "<:thumbs_up:703358705089118299> Setting Updated",
            "Embed filtering is now **{0}**.".format(["disabled", "enabled"][is_enabled])
        )
    else:
        await functions.send_error(message.channel, "Expected 'on' or 'off'", "manage-embeds")